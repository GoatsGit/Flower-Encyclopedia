...
pip install tensorflow-gpu 명령을 실행해 gpu를 사용하는 tensorflow를 설치하였고, 설치가 완료된 후 cmd 창에 python 을 입력 후 import tensorflow as tf 를 입력하여 오류가 발생하는지 확인한 후 오류가 발생하지 않아 tensorflow의 설치가 완료되었음을 확인하였다.
